var GKWebConnectorLayerV8 = GK_WebConnectorLayer.extend({
    //***********变量*************
    mWidth: null,
    mHeight: null,
    mXOffset: null,
    ...
    createLayer: function(aTitle, aWaitDuration, aRoomGroup) {
        ...
        nodeLink.href = "http://activity.goooku.com/scene/common/screen/activityConnectionV1/css/main.css";
    ...
    showLayer: function(aCallback) {
        ...
        nodeli.style.cssText =
            ...
            'background-image:url("http://activity.goooku.com/scene/common/screen/activityConnectionV1/img/main_bg.png");';
        ...
        if(window.localStorage){
            guiyiQR = window.localStorage.getItem('guiyiQR');
        }
        _.el('.scene-container').html(
            ...
            '<div class="content">' +
            '<div class="connect">' +
            '<div id="qrcode" style="background-image: url('+guiyiQR+');">'+
            '</div>' +
            '</div>' +
            '</div>'
        );
        ...
        requestAnimationFrame(function(){
            ...
            var activityHeight = Math.floor((_.el('.activity')[0].clientHeight-_.el('.activity')[0].clientWidth*0.03) / 3);
        ...
        //添加等待动画
        var _num = parseInt(window.getComputedStyle(_.el('.header')[0], null).height.replace('px', '')) / 5;
        var waitDiv = document.createElement("div");
        waitDiv.className = 'wait-animate';
        waitDiv.innerHTML = "<div class='text' style='font-size:"+_num+"px;margin-top: -"+_num+"px'>等待小伙伴加入。。。</div>";
        _.el('.scene-container')[0].appendChild(waitDiv);

        //进行数量检查
        this.avatarRefreshInterval = window.setInterval(function() {
            if(_.el('#avatar')[0].childNodes.length > 12) {
    ...
    deleteLayer: function() {
        ...
        cc.game.run();
        var httpRequest=new GK_HttpRequest(null,this.mRoomGroup);httpRequest.sendActivityStart(function(aErr){if(aErr){}});
